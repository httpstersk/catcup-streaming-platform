...
- **Interface Stability**: The `lib/pipeline` module is shared by both web and CLI. Maintain backward compatibility for `runPipeline` signature.
...
- Import types from the correct SDK subpaths (e.g., `Tool` from `"ai"`, not from internal paths)
- Type function parameters and return values explicitly
- Use Zod schemas for runtime validation at API boundaries

### Constants and Configuration
- Extract magic numbers to named constants in `lib/pipeline/constants.ts`
...
- Group related functionality in subdirectories (e.g., `lib/pipeline/*`, `lib/threads/*`)
